...
def summary(db: Session) -> dict:
    ...
    failed_payments = (
        db.scalar(select(func.count(Payment.id)).where(Payment.status == PaymentStatus.FAILED)) or 0
    )
    active_vendors = (
        db.scalar(select(func.count(Vendor.id)).where(Vendor.status == "APPROVED")) or 0
    )
    ...
    try:
        ...
        rows_q = db.execute(
            select(
                ...
                .label("orders"),
                func.coalesce(
                    func.sum(LedgerEntry.amount_taka).filter(
                        LedgerEntry.entry_type == LedgerEntryType.PAYMENT
                    ),
                    0,
                ).label("sales"),
        ...
        by_day = {
            str(r.day): {
                "orders": r.orders,
                "sales_taka": int(r.sales),
                "refunds_taka": int(r.refunds),
            }
            for r in rows_q
        }
        return [
            {
                ...
                "orders": by_day.get((since + timedelta(days=i)).isoformat(), {}).get("orders", 0),
                "sales_taka": by_day.get((since + timedelta(days=i)).isoformat(), {}).get(
                    "sales_taka", 0
                ),
                "refunds_taka": by_day.get((since + timedelta(days=i)).isoformat(), {}).get(
                    "refunds_taka", 0
                ),
            }
            for i in range(days)
        ...
        buckets: dict[str, dict] = defaultdict(
            lambda: {"orders": set(), "sales_taka": 0, "refunds_taka": 0}
        )
        for e in entries:
...
def vendor_sales(db: Session, vendor_id: str, since: date | None = None) -> dict:
    ...
    q = (
        select(func.coalesce(func.sum(OrderItem.subtotal_taka), 0))
