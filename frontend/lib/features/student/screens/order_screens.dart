  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: orders.when(
        ...
        data: (list) {
          if (list.isEmpty) {
            return EmptyView(icon: Icons.receipt_long_rounded, message: 'No orders yet. Browse vendors to place your first order.');
          }
                ...
                return AppCard(
                  ...
                  child: Row(children: [
                    Container(
                        ...
                        child: Icon(Icons.receipt_outlined, size: 18, color: AppColors.textSecondary)),
                    SizedBox(width: 12),
...
class _OrderTrackingScreenState extends ConsumerState<OrderTrackingScreen> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
      appBar: AppBar(
        leading: IconButton(icon: Icon(Icons.arrow_back_rounded), onPressed: () => context.go('/student/orders')),
        ...
        actions: [
          Container(
            ...
            child: Row(children: [Icon(Icons.circle, size: 6, color: AppColors.success), SizedBox(width: 6), Text('LIVE', style: TextStyle(color: AppColors.success, fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 0.5))]),
          )
        ],
        ...
        data: (o) => ListView(padding: EdgeInsets.all(16), children: [
          AppCard(
            child: Column(children: [
              ...
              if (o.createdAt != null) ...[
                SizedBox(height: 10),
                Text(formatDateTime(o.createdAt!), style: TextStyle(color: AppColors.textTertiary, fontSize: 11)),
              ]
            ]),
          ),
          SizedBox(height: 12),
          AppCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              SectionHeader(title: 'Journey', subtitle: 'Auto-refreshes every 5 seconds'),
              SizedBox(height: 14),
              if (stepIndex >= 0)
                ...List.generate(_steps.length, (i) {
                  ...
                  return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    SizedBox(width: 12),
                    Expanded(child: Padding(padding: EdgeInsets.only(bottom: 14), child: Text(label, style: TextStyle(color: cur ? AppColors.textPrimary : done ? AppColors.textSecondary : AppColors.textTertiary, fontWeight: cur ? FontWeight.w600 : FontWeight.w400, fontSize: 13)))),
                    if (cur)
                      Container(padding: EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(color: AppColors.brandSubtle, borderRadius: BorderRadius.circular(6)), child: Text('Now', style: TextStyle(color: AppColors.brand, fontSize: 10, fontWeight: FontWeight.w700))),
                  ]);
                })
              else
                Text('Order status: ${o.status}', style: TextStyle(color: AppColors.textSecondary, fontSize: 13)),
            ]),
          ),
          SizedBox(height: 12),
          AppCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Items', style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w600, fontSize: 13)),
              SizedBox(height: 8),
              ...o.items.map((it) => Padding(
                  ...
                  child: Row(children: [
                    ...
                    SizedBox(width: 12),
                    Text(taka(it.subtotal), style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w600, fontSize: 12)),
                  ]))),
              Divider(height: 16),
          ...
          if (['PENDING_PAYMENT', 'PAID'].contains(o.status))
            OutlinedButton.icon(
              onPressed: () async {
                final confirmed = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(title: Text('Cancel order?'), content: Text('This cannot be undone.'), actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text('Keep order')), FilledButton(onPressed: () => Navigator.pop(ctx, true), style: FilledButton.styleFrom(backgroundColor: AppColors.error), child: Text('Cancel'))]));
              ...
              style: OutlinedButton.styleFrom(side: BorderSide(color: AppColors.errorBorder), foregroundColor: AppColors.error, minimumSize: Size.fromHeight(44)),
            ),
        ]),
      ),
    );
  ...
  Widget _metric(String k, String v, Color c, {bool isCode = false}) => Column(children: [
        Text(k.toUpperCase(), style: TextStyle(color: AppColors.textTertiary, fontSize: 10, fontWeight: FontWeight.w600, letterSpacing: 0.6)),
      ...
      body: order.when(
        ...
        data: (o) => SingleChildScrollView(
          ...
          child: Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Center(child: Container(padding: EdgeInsets.symmetric(horizontal: 10, vertical: 5), decoration: BoxDecoration(color: AppColors.brandSubtle, borderRadius: BorderRadius.circular(6)), child: Text('IUB PAY', style: TextStyle(color: AppColors.brand, fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 1)))),
              SizedBox(height: 8),
              Center(child: Text(o.orderNumber, style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w600, fontSize: 14))),
              if (o.createdAt != null) Center(child: Text(formatDateTime(o.createdAt!), style: TextStyle(color: AppColors.textTertiary, fontSize: 11))),
              SizedBox(height: 14),
              Divider(height: 1),
              SizedBox(height: 14),
              ...o.items.map((it) => Padding(padding: EdgeInsets.symmetric(vertical: 5), child: Row(children: [Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(it.name, style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w500, fontSize: 13)), Text('${it.quantity} × ${taka(it.unitPrice)}', style: TextStyle(color: AppColors.textTertiary, fontSize: 11))])), Text(taka(it.subtotal), style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w600))]))),
              SizedBox(height: 10),
              Divider(height: 1),
              SizedBox(height: 10),
              _row('Subtotal', o.subtotal), SizedBox(height: 4), _row('Service fee', o.serviceFee),
              SizedBox(height: 8),
              Divider(height: 1),
              SizedBox(height: 8),
              ...
              Container(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Payment: ${o.latestPayment?.status ?? "NOT PAID"}', style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                    if (o.latestPayment != null) Text('Txn: ${o.latestPayment!.id}', style: TextStyle(color: AppColors.textTertiary, fontSize: 11)),
                    SizedBox(height: 8),
                    Center(child: Container(padding: EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(10), border: Border.all(color: AppColors.borderStrong)), child: Text('Pickup  ${o.pickupCode}', style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w700, letterSpacing: 2.5, fontSize: 13)))),
                  ])),
            ]),
          ),
        ),
