            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              AppCard(
                ...
                child: Column(children: [
                  ...
                  Text('Order $_shortOrderId…', style: const TextStyle(color: AppColors.textTertiary, fontSize: 11)),
              ...
              if (_busy && _amount != null)
                ...
                PrimaryButton(
                  ...
                  outlined: true,
                  busy: _busy,
                  onPressed: canAct ? () => _complete(success: false) : null,
                ),
                const SizedBox(height: 10),
                AppCard(
                  onTap: canAct
                  ...
                  child: Row(children: [
                    Icon(Icons.hourglass_top_rounded, size: 18, color: canAct ? AppColors.textSecondary : AppColors.textTertiary),
                    const SizedBox(width: 12),
                    Expanded(
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text('Delayed success (~3s)',
                          style: TextStyle(color: canAct ? AppColors.textPrimary : AppColors.textTertiary, fontWeight: FontWeight.w600, fontSize: 13)),
                      const Text('Simulates network latency', style: TextStyle(color: AppColors.textTertiary, fontSize: 11)),
                    ])),
                    Icon(Icons.chevron_right_rounded, color: canAct ? AppColors.textTertiary : AppColors.borderStrong, size: 18),
                  ]),
                ),
                if (_paymentId == null && !_busy && _error != null) ...[
                  const SizedBox(height: 10),
                  PrimaryButton(label: 'Retry', icon: Icons.refresh_rounded, outlined: true, onPressed: _createPayment),
                ],
              ],
              if (_error != null) ...[
                const SizedBox(height: 14),
